...
const Hero = () => {
  ...
  return (
    <section className="relative min-h-screen h-auto lg:h-screen overflow-hidden pt-24">
      ...
      <div className="relative lg:absolute lg:right-40 lg:bottom-0 lg:h-full w-full max-w-[1440px] pointer-events-none flex items-end justify-center lg:justify-end z-20 mt-12 lg:mt-0">
        <div className="relative h-[95vw] sm:h-[80vw] md:h-[55vw] lg:h-[85vh] w-auto lg:translate-x-[22%] scale-[1.1] translate-y-4 sm:translate-y-0 sm:scale-95 md:scale-100 origin-bottom">
          ...
          >
            <div className="backdrop-blur-xl bg-white/20 border border-white/30 rounded-xl p-2 sm:p-3 shadow-2xl shadow-black/15 flex flex-col gap-1 sm:gap-1.5 w-28 sm:w-44 scale-90 sm:scale-100">
              ...
              {noteItems.map(({ Icon, label, accent }) => (
                ...
                >
                  <Icon size={11} strokeWidth={2} className={`shrink-0 ${accent ? 'text-white' : 'text-purple-700'}`} />
                  {label}
                </div>
              ))}
            </div>
          ...
          >
            <div className="backdrop-blur-xl bg-white/20 border border-white/30 rounded-xl p-2 sm:p-3 shadow-2xl shadow-black/15 flex flex-col gap-2 sm:gap-2.5 w-28 sm:w-40 scale-85 sm:scale-100">
              ...
              <div className="flex items-center justify-between px-0.5">
                <span className="text-[9px] sm:text-[11px] text-text-primary/60">Auto-switch</span>
          ...
          >
            <div className="backdrop-blur-xl bg-white/20 border border-white/30 rounded-xl p-2 sm:p-3 shadow-2xl shadow-black/15 flex flex-col gap-1.5 sm:gap-2 w-28 sm:w-44 scale-85 sm:scale-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[7px] sm:text-xs font-semibold text-text-primary">All devices synced</span>
              </div>
                ...
                {deviceItems.map(({ Icon, label }) => (
                  <div key={label} className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <Icon size={10} strokeWidth={2} className="text-text-primary/50 shrink-0" />
                      <span className="text-[7px] sm:text-[11px] text-text-primary/70">{label}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <RefreshCw size={8} strokeWidth={2.5} className="text-green-500" />
                      <span className="text-[6px] sm:text-[9px] text-green-600 font-semibold">Synced</span>
                    </div>
                  </div>
                ))}
              </div>
