...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <NavLink to="/" className="flex items-center group">
          ...
          <span className={`text-2xl font-bold tracking-tight transition-colors duration-300 ${
            scrolled ? 'text-primary' : 'text-text-primary'
          }`}>
            Notebean
          </span>
        </NavLink>
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`lg:hidden fixed inset-0 top-[72px] bg-white/95 backdrop-blur-xl transition-all duration-500 ease-in-out ${
        ...
        <div className="flex flex-col p-8 gap-6 h-full">
          {navLinks.map((link, index) => (
            <NavLink
              key={link.name}
              to={link.path}
              className="text-2xl font-bold text-text-primary hover:text-primary transition-colors py-2 border-b border-surface"
          ...
          <button className="mt-4 px-8 py-4 rounded-2xl bg-primary text-white font-bold text-lg shadow-xl hover:bg-secondary transition-colors">
            Get Started
          </button>
        </div>
      </div>
